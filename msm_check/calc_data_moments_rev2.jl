...
function mnt_var_leftbh(df::AbstractDataFrame, lnWname::Symbol, cage9::Symbol,
					  	...
					  	XQJMnames::AbstractVector{Symbol}, mnt_len::Int; nboot::Int = 200)

	leftbh_bsmnt_mat = bs_leftbh_mnts(df, lnWname, cage9, XTnames, XLnames, XFnames,
					  	  		  	  XMnames, XQJMnames, mnt_len; nboot = nboot)
	lb_mnt_nobs = dtmnts_nobs_leftbh(df, lnWname, XTnames, XLnames, XFnames, XMnames, XQJMnames)
...
function bs_leftbh_mnts(df::AbstractDataFrame, lnWname::Symbol, cage9::Symbol,
	...
	for b = 1:nboot
		...
		view(leftbh_bs_mat, :, b) .= data_moments_leftbh(bsdf, lnWname, cage9, XTnames,
														  XLnames, XFnames, XMnames, XQJMnames)
	end
...
function data_moments_leftbh(df::AbstractDataFrame, lnWname::Symbol, cage9::Symbol,
							 XTnames::AbstractVector{Symbol}, XLnames::AbstractVector{Symbol},
							 XFnames::AbstractVector{Symbol}, XMnames::AbstractVector{Symbol},
							 XQJMnames::AbstractVector{Symbol})
    ...
    # --- (4) data moments for lnW_mig and lnW_lft ---
	...
	# --- (7) data moments for XT'lnW and E(XF XT') ---
	XT_lnW_mnt = colwise(x -> mean(x .* lnW, weights(wgtvec)), view(df, XTnames))

	XF_XT_mnt = Matrix{Float64}(view(df, :, XFnames))' *
					(Matrix{Float64}(view(df, :, XTnames)) .* wgtvec) / sum(wgtvec)
	XF_XT_mnt = vec(XF_XT_mnt)

	# --- (8) data moments for XT'XQJ_mig and XT'XQJ_lft ---
	wgtvec_lft = view(wgtvec, leftbh .== 1)
	wgtvec_mig = view(wgtvec, leftbh .== 0)
	XT_XQJ_mig_mnt = Matrix{Float64}(view(df, leftbh .== 0, XTnames))' *
					(Matrix{Float64}(view(df, leftbh .== 0, XQJMnames)) .* wgtvec_mig) / sum(wgtvec_mig)
	XT_XQJ_mig_mnt = vec(XT_XQJ_mig_mnt)

	XT_XQJ_lft_mnt = Matrix{Float64}(view(df, leftbh .== 1, XTnames))' *
...
function dtmnts_nobs_leftbh(df::AbstractDataFrame, lnWname::Symbol,
	...
	# --- (8) data moments for XT'XQJ_mig and XT'XQJ_lft ---
	XT_XQJ_mig_mnt_n = swt_mig * ones(nXT * nXQJ)
