...
function individual_mnts!(mntvec, mnt_range, mktshare, lftshare, lftpr_is,
	...
	view(mntvec, mnt_range[8]) .= vec(view(xf_xt_p, 2:(nxf-1), :)) #<-- NOTE: drop constant
	...
	#
	# vcat(lftshare(77*2), xm_mnt(15), xfw_mnt_mig(7), xfw_mnt_lft(7),
	# 	   xl_mnt_lft(4), xt_mnt_lft(5), xt_lnw_mnt(5),
	#
	# 	   xf_xt(30), xt_xqj_mig(10), xt_xqj_lft(10)
	#
	# 	   zm_mnt_mig(2), zm_mnt_lft(2),
	# 	   zlnw_mnt(2), zxqj_mig_mnt(4), zxqj_lft_mnt(4), xq_lnq_mig(7), xqj_lnq_mig(2),
	# 	   xq_lnq_lft(7), xqj_lnq_lft(2), lnwq_mig(1), lnwq_lft(1), lnq2_mig(1), lnq2_lft(1))
	#
end
...
function unpack_parm_msm(parm, XT::AbstractMatrix{T}, XL::AbstractMatrix{T},
	 ...
	 mnt_idx = [nalt, nxm, nxf, nxf, nxl, nxt, nxt, (nxf - 2) * nxt,
