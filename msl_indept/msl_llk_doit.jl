...
using FileIO, DataFrames, CSV, StatsFuns, Statistics, StatsBase, LinearAlgebra, GLM
...
nparm = size(XT, 1) + size(XL, 1) + size(XM, 1) + size(XF, 1) + 2

# xt_init = [3.0; lft_init[2:size(XT, 1)]]
xt_init = lft_init[2:(size(XT, 1) + 1)]
...
@time mig_leftbh_llk(initval, initdel, YL, YM, lnW, lnP, XbQ, XQJ_mig,
			 		 XT, XL, XM, XFW, USHK, wgt, nind, nalt, nsim, dgvec, 1)
# 444022.5091; 5.1s
